...
export default function ReportsPage() {
  ...
  const [topProducts, setTopProducts] = useState<
    { team: string; size: string; qty: number; revenue: number }[]
  >([])
  ...
  const [staleProducts, setStaleProducts] = useState<
    { team: string; size: string; days: number | null }[]
  >([])
  ...
  async function loadTopProducts() {
    ...
    const { data, error } = await supabase
      .from('sale_items')
      .select('quantity, unit_price, product_id, sales!inner(created_at, sale_status), products(team, size)')
      .gte('sales.created_at', startTs)
      .lte('sales.created_at', endTs)
      .neq('sales.sale_status', 'cancelada')
    if (error) throw error
    ...
    const map = new Map<string, { team: string; size: string; qty: number; revenue: number }>()
    ;((data as TopProductItem[] | null) ?? []).forEach((item) => {
      const team = item.products?.team ?? 'Produto removido'
      const size = item.products?.size ?? '-'
      const key = `${item.product_id ?? team}-${size}`
      const entry = map.get(key) ?? { team, size, qty: 0, revenue: 0 }
  ...
  async function loadStaleProducts() {
    ...
    const [productsRes, movementsRes] = await Promise.all([
      supabase.from('products').select('id, team, size').eq('archived', false),
      supabase
    ...
    const stale = (productsRes.data ?? [])
      ...
      .map((p) => {
        ...
        return { team: p.team, size: p.size, days }
      })
        ...
        {topProducts.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {topProducts.map((p, i) => (
                  <tr key={`${p.team}-${p.size}-${i}`} className="border-b border-gray-50">
