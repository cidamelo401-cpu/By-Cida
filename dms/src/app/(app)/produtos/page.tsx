...
import type { Database, ProductModel, ProductSize, ProductStatus, ProductVersion } from '@/types/database'
...
export default function ProductsPage() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      setLoading(true)
      try {
        const query = supabase.from('products').select('*').order('team')
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((p) => {
      if (term) {
        const matchesTerm =
          p.team.toLowerCase().includes(term) || (p.sku ?? '').toLowerCase().includes(term)
  ...
  }, [products, search, sizeFilter, modelFilter, versionFilter, statusFilter])
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <FilterChip
            label="Versão"
            value={versionFilter}
            options={Object.entries(VERSION_LABELS).map(([value, label]) => ({ value, label }))}
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description={
              products.length === 0
                ? 'Cadastre a primeira camisa do seu estoque.'
      ...
      <Card className="overflow-hidden h-full flex flex-col">
        ...
        <div className="p-3 flex flex-col gap-1 flex-1">
          ...
          <p className="text-xs text-gray-500">
            {product.season ?? '—'} · {MODEL_LABELS[product.model]} / {VERSION_LABELS[product.version]}
          </p>
