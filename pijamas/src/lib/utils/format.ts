...
/** Generate a SKU from name, collection, model, fabric and size */
export function generateSKU(name: string, collection: string, model: string, fabric: string, size: string): string {
  const slug = (s: string) =>
    s
      .normalize('NFD')
      .replace(/[̀-ͯ]/g, '')
  ...
  return [slug(name), slug(collection), slug(model), slug(fabric), slug(size)].filter(Boolean).join('-')
}
