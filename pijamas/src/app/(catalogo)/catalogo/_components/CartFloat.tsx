...
export default function CartFloat() {
  ...
  function handleWhatsApp() {
    const baseUrl = typeof window !== 'undefined' ? window.location.origin : 'https://pijamas-digital.vercel.app'
    const lines = items.map(
      (item, i) => `${i + 1}. ${item.name} — ${item.product_type} — Tam. ${item.sizeLabel} — ${formatCurrency(item.price)}\n   ${baseUrl}/catalogo/${item.id}`
    )
    const message = `Oi! Tenho interesse nos seguintes pijamas:\n\n${lines.join('\n\n')}\n\nTotal: ${formatCurrency(total)}`
              ...
              {items.map((item) => (
                <div key={item.id} className="flex gap-3 bg-[#0A0A0A] rounded-xl p-3 border border-white/5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-gray-500">{item.product_type} · Tam. {item.sizeLabel}</p>
