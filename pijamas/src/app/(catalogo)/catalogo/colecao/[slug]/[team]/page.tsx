...
export default function CollectionItemsPage({ params }: { params: Promise<{ slug: string; team: string }> }) {
  ...
  const typeName = decodeURIComponent(teamSlug)

  const [products, setProducts] = useState<Product[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')

  const badges = useTeamBadges([typeName])
    ...
    async function load() {
      ...
      try {
        ...
        if (collectionName === 'Outros') {
          query = query.or('collection.is.null,collection.eq.')
        } else {
    ...
    <CatalogShell>
      ...
      <div className="mx-auto max-w-6xl px-4 pb-6">
        <div className="flex items-center gap-4">
          ...
          <TeamBadge label={typeName} badgeUrl={badges[typeName]} size="lg" />
          <div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-4">
        {error ? (
          <div className="rounded-xl bg-red-950/40 border border-red-900 p-4 text-center">
            <p className="text-sm font-medium text-red-400">Erro ao carregar pijamas</p>
          ...
          <div className="flex items-center justify-center py-20">
            ...
            <span className="ml-3 text-sm text-gray-500">Carregando pijamas...</span>
          </div>
        ) : grouped.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="mt-4 font-semibold text-white">Nenhuma pijama encontrada</p>
          </div>
        ) : (
          <>
