...
const KIDS_SIZES: ProductSize[] = ['2', '4', '6', '8', '10', '12', '14']
...
export default function CatalogoPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeCollection) {
      if (activeCollection === '__outros__') {
        result = result.filter((p) => !p.collection?.trim())
      } else {
        result = result.filter((p) => p.collection?.trim() === activeCollection)
      }
    }
  ...
  const collections = useMemo(() => {
    ...
    for (const p of products) {
      const league = p.collection?.trim()
      if (league) {
        map.set(league, (map.get(league) ?? 0) + 1)
      }
    }
    const hasOthers = products.some((p) => !p.collection?.trim())
    const result = Array.from(map.entries())
      .sort((a, b) => b[1] - a[1])
      .map(([name, count]) => ({ name, count }))
    if (hasOthers) {
      const othersCount = products.filter((p) => !p.collection?.trim()).length
  ...
  const teamNames = useMemo(() => {
    const set = new Set(filteredProducts.map((p) => p.name))
  ...
  const groupedShirts = useMemo(() => {
    ...
    const teamProducts = filteredProducts.filter((p) => p.name === activeTeam)
              ...
              >
                <div className={`w-16 h-16 rounded-full bg-[#1A1A1A] border-2 transition-colors flex items-center justify-center overflow-hidden ${
                  ...
                  {badges[name] ? (
                    ...
                    <TeamBadge label={name} badgeUrl={undefined} size="md" />
                  )}
                </div>
        ...
        {error ? (
          ...
          groupedShirts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              <p className="mt-4 font-semibold text-white">Nenhum pijama encontrado</p>
            </div>
          ) : (
            <>
                  ...
                  >
                    <div className="bg-[#1A1A1A] rounded-2xl overflow-hidden border border-white/5 hover:border-[#C9A84C]/40 transition-colors h-full flex flex-col">
                      ...
                      <div className="p-3 flex flex-col gap-1.5 flex-1">
                        <p className="font-semibold text-white text-sm leading-tight line-clamp-2">{shirt.name}</p>
                        <p className="text-[11px] text-gray-500">
                          {shirt.color ?? ''} · {MODEL_LABELS[shirt.model]}
                        </p>
            ...
            <>
              ...
              <div className="mt-8">
                <div
                  ...
                  tabIndex={0}
                  onClick={() => {
                    const msg = 'Olá! Não encontrei a pijama que procuro no catálogo. Vocês conseguem me ajudar?'
