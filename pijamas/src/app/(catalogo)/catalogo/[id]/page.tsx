...
import { MODEL_LABELS, FABRIC_LABELS,  } from '@/lib/constants/products'
...
const SIZE_ORDER: ProductSize[] = ['2', '4', '6', '8', '10', '12', '14', 'PP', 'P', 'M', 'G', 'GG', '2XG', '3XG']
...
export default function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        if (data.color) {
          query = query.eq('color', data.color)
        }
  ...
  const handleShare = async () => {
    const shareText = `Olha que linda essa pijama que eu vi na Pijamas Digital! 🔥⚽`
    const shareUrl = typeof window !== 'undefined' ? window.location.href : ''
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: product?.name ?? 'Pijamas Digital',
          text: shareText,
          url: shareUrl,
        })
      } catch {
  ...
  const productPhotos: string[] = product.photos?.length
    ? product.photos
    : product.photo_url
      ? [product.photo_url]
      : []
  ...
  const sizeLabel = sizeForMessage
  const productUrl = typeof window !== 'undefined' ? window.location.href : `https://pijamas-digital.vercel.app/catalogo/${id}`
  const modelLabel = MODEL_LABELS[product.model]
  const versionLabel = FABRIC_LABELS[product.fabric]
  const whatsappMessage = `Oi! Vi essa pijama no catálogo e tenho interesse!\n\n⚽ ${product?.name}\n📋 ${modelLabel} · ${versionLabel}\n📏 Tamanho ${sizeLabel}\n\n🔗 ${productUrl}`
  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = `Oi! Meu nome é ${leadName.trim()}. Vi essa pijama no catálogo e tenho interesse!\n\n⚽ ${product!.name}\n📋 ${MODEL_LABELS[product!.model]} · ${FABRIC_LABELS[product!.fabric]}\n📏 Tamanho ${sizeLabel}\n\n🔗 ${productUrl}`
  ...
  const isKids = SIZE_ORDER.indexOf(product.size) < 7 || siblings.some((s) => SIZE_ORDER.indexOf(s.size) < 7)
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A]">
      ...
      <main className="mx-auto max-w-4xl px-4 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {product.collection ?? ''}{product.collection && product.color ? ' · ' : ''}{product.color ?? ''}
            </p>

            <div className="mt-4 flex flex-wrap gap-2">
              <InfoChip>{MODEL_LABELS[product.model]}</InfoChip>
              <InfoChip>{FABRIC_LABELS[product.fabric]}</InfoChip>
            </div>
            ...
            {product.status === 'sob_encomenda' ? (
              <>
                <div className="mt-6 rounded-xl bg-blue-950/30 border border-blue-800/40 p-4">
                  <p className="text-sm text-blue-300 font-semibold mb-1">Sob Encomenda</p>
                ...
                <button
                  type="button"
                  onClick={() => {
                    const msg = `Olá! Vi essa pijama no catálogo e gostaria de consultar a disponibilidade sob encomenda.\n\n⚽ ${product?.name}\n📋 ${modelLabel} · ${versionLabel}\n\n🔗 ${productUrl}`
                    window.location.assign(getWhatsAppLink(WHATSAPP_NUMBER, msg))
                  }}
                  className="mt-4 flex items-center justify-center gap-3 w-full px-6 py-4 rounded-xl bg-[#25D366] text-white font-bold uppercase tracking-wide text-sm hover:bg-[#1fb855] transition-colors cursor-pointer border-0"
                >
              ...
              <>
                {/* WhatsApp CTA — button puro, sem <a> para evitar interceptação do Next.js */}
                <button
                  type="button"
                  onClick={() => { window.location.assign(whatsappUrl) }}
                  className="mt-6 flex items-center justify-center gap-3 w-full px-6 py-4 rounded-xl bg-[#C9A84C] text-black font-bold uppercase tracking-wide text-base hover:bg-[#b8983f] transition-colors cursor-pointer border-0"
                >
                  <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    addItem({
                      ...
                      name: product?.name ?? '',
                      product_type: MODEL_LABELS[product.model],
                      size: sizeForMessage,
                      sizeLabel: sizeLabel,
            ...
            <p className="mt-3 text-xs text-gray-600 text-center">
              Ao clicar, você será redirecionado para o WhatsApp da Pijamas Digital
            </p>
                  ...
                  <div className="overflow-x-auto mb-4">
                    <table className="w-full text-sm">
                      ...
                      <tbody className="text-gray-300">
                        {[
                          { size: '2', age: '1-2 anos', height: '80-92 cm' },
                          { size: '4', age: '3-4 anos', height: '98-104 cm' },
                          { size: '6', age: '5-6 anos', height: '110-116 cm' },
                          { size: '8', age: '7-8 anos', height: '122-128 cm' },
                          { size: '10', age: '9-10 anos', height: '134-140 cm' },
                          { size: '12', age: '11-12 anos', height: '146-152 cm' },
                          { size: '14', age: '13-14 anos', height: '158-164 cm' },
                        ].map((row) => (
                          <tr
                            key={row.size}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/70 backdrop-blur-sm" onClick={() => setShowLeadModal(false)}>
          <div className="bg-[#1A1A1A] rounded-t-2xl sm:rounded-2xl border border-white/10 w-full sm:max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-white/5">
              <div>
                <h3 className="text-lg font-bold text-white">Quero essa pijama!</h3>
            ...
            <form onSubmit={handleLeadSubmit} className="p-5 space-y-4">
              ...
              <p className="text-[10px] text-gray-600 text-center">
                Seus dados são usados apenas para contato sobre esta pijama.
              </p>
            </form>
          </div>
        </div>
      ...
      <button
        type="button"
        onClick={() => { window.location.assign(whatsappUrl) }}
        className="fixed bottom-5 right-5 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg hover:bg-[#1ebe5a] transition-colors cursor-pointer border-0"
    ...
    <header className="sticky top-0 z-30 bg-[#0A0A0A]/95 backdrop-blur border-b border-white/5">
      <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
        <Link href="/catalogo" className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-bold leading-tight text-white">
              Pijamas <span className="text-[#C9A84C]">Digital</span>
            </p>
            <p className="text-[10px] text-gray-500">Pijamas</p>
          </div>
        </Link>
        <a
          href="https://www.instagram.com/pijamasdigital"
          target="_blank"
        ...
        >
          ...
          <span className="hidden sm:inline">@pijamasdigital</span>
        </a>
      </div>
    </header>
  )
