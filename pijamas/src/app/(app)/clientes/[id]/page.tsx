  ...
  const stats = useMemo(() => {
    ...
    for (const sale of sales) {
      for (const item of sale.sale_items ?? []) {
        if (item.products?.name) styles.add(item.products.name)
      ...
      <div className="flex flex-col gap-5 pb-10">
        <Card className="p-4 flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            {customer.preferred_style && <Badge status="primary">{customer.preferred_style}</Badge>}
        ...
        {(stats.styles.length > 0 || stats.sizes.length > 0) && (
          <Card className="p-4 flex flex-col gap-3">
            {stats.styles.length > 0 && (
              <div>
                <p className="text-xs font-medium text-gray-500 mb-1.5">Produtos comprados</p>
                <div className="flex flex-wrap gap-1.5">
                  {stats.styles.map((style) => (
                    <Badge key={style} status="primary">
                      {style}
                    </Badge>
                  ))}
                </div>
