  ...
  async function uploadAll() {
    ...
    for (const item of toUpload) {
      ...
      try {
        ...
        const fileName = `${item.product.name.toLowerCase().replace(/\s+/g, '-')}-${item.product.model}-${item.product.size}-${Date.now()}.${ext}`
        ...
        console.error(`Erro ao enviar foto de ${item.product.name}:`, err)
        setItems((prev) =>
          prev.map((i) =>
            i.product.id === item.product.id ? { ...i, uploading: false } : i
          )
        )
        toast.error(`Erro: ${item.product.name} (${item.product.size})`)
      }
    }
