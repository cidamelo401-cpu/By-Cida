...
type Sale = Database['public']['Tables']['sales']['Row'] & {
  ...
  sale_items: { id: string; quantity: number; products: { name: string; size: string; sku: string | null } | null }[]
}
  ...
  async function loadSales() {
    ...
    try {
      const { data, error } = await supabase
        .from('sales')
        .select('*, customers(name, whatsapp), sale_items(*, products(name, size, sku))')
