...
type SaleItem = Database['public']['Tables']['sale_items']['Row'] & {
  products: { name: string; size: string; sku: string | null } | null
}
...
export default function VendaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function loadAll() {
    ...
    try {
      const [{ data: saleData, error }, { data: paymentsData }, { data: historyData }] = await Promise.all([
        supabase
          .from('sales')
          .select('*, customers(*), sale_items(*, products(name, size, sku))')
          .eq('id', id)
          .single(),
  ...
  const reservationMessage = customer
    ? `Olá, ${customer.name}! Sua pijama ${firstItem?.products?.name ?? ''} ${firstItem?.products?.size ?? ''}, está reservada até ${
