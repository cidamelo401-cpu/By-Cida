...
export default function StockMovementsPage() {
  ...
  async function loadProducts() {
    const { data, error } = await supabase.from('products').select('id, name, size').order('name')
    if (error) {
      toast.error('Erro ao carregar produtos.')
      return
    }
    setProducts((data ?? []).map((p) => ({ id: p.id, name: p.name, size: p.size })))
  }
  ...
  async function loadMovements(pageIndex: number, replace: boolean) {
    ...
    try {
      let query = supabase
        ...
        .select('*, products(name, size), profiles:created_by(full_name)')
  ...
  function handleExport() {
    exportToCSV(
      movements.map((m) => ({
        ...
        Produto: `${m.product_name ?? '-'} (${m.product_size ?? '-'})`,
                ...
                {movements.map((m) => (
                  <tr key={m.id} className="border-b border-gray-50">
                    ...
                    <td className="py-2 px-4 whitespace-nowrap">
                      {m.product_name ?? 'Produto removido'} {m.product_size ? `(${m.product_size})` : ''}
                    </td>
